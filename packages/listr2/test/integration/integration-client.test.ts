...
import {
  ...
  name as packageName
} from 'rootverse+listr2:package.json';
...
const debug = createDebugLogger({ namespace: 'listr2' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+listr2:package.json'))),
    packageName,
    packageExports
  );
});
