...
var Execute = function(...args)
{
    if(this._instantiatedAlready)
    {
        if(args.length < 1 || args.length > 3)
            throw `Execute expects between 1 and 3 arguments, ${args.length} were passed in instead`;
    ...
    else
    {
        if(!(this instanceof Execute))
            throw 'an instance of the Execute object must be constructed with the "new" operator';
 ...
 * @return {Boolean}                If the command succeeded or failed
 */
Execute.execute = function(command, description, ignoreErrors)
{
    // Echo message
    if(description)
 ...
 * @return {Boolean}                If the command succeeded or failed
 */
Execute.butIgnoreErrors = function(command, description)
{
 ...
 * @return {Boolean}                If the command succeeded or failed
 */
Execute.withoutLogging = function(command, description, ignoreErrors)
{
