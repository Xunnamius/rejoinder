...
class Execute
{
     ...
     *
     * @param {Echo} echo A fully instantiated Echo object instance
     */
    constructor(echoInstance)
    {
        if(!(echoInstance instanceof Echo))
            throw 'Execute expects 1 argument of type Echo to be passed in during instantiation';
    ...
    now(command, description, ignoreErrors)
    {
        ...
        if(result.code !== 0 && !ignoreErrors)
        {
            if(description || this._echo.beVerbose)
                (!description ? this._echo.ifVerbose : this._echo.withPostfix.error)('FAILED!');
    ...
    {
        var prevLog = this._echo.logFilePath;
